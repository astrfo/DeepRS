...
class DQN_RND:
    def __init__(self, model=QNet, **kwargs):
        ...
        self.rnd_model = RNDNet(input_size=self.state_space, hidden_size=self.hidden_size, output_size=self.hidden_size).float()
        self.rnd_model.to(self.device)
        self.rnd_model_target = RNDNet(input_size=self.state_space, hidden_size=self.hidden_size, output_size=self.hidden_size).float()
        self.rnd_model_target.to(self.device)
        self.optimizer = optim.Adam(self.model.parameters(), lr=self.adam_learning_rate)
        self.rnd_optimizer = optim.Adam(self.rnd_model.parameters(), lr=self.adam_learning_rate)
    ...
    def update(self, state, action, reward, next_state, done):
        ...
        predictor_feature = self.rnd_model(s)
